...
import torch
import numpy as np
from ..data.loader import TextImageDataset, PretrainDataset
...
def train(params, model):
    ...
    mask = torch.tensor(
        (rr > 0.4) & (rr < 3), device=params["device"], dtype=torch.float32
    ).reshape(1, 1, params["image_size"], params["image_size"])
    ...
    for ep in range(starting_epoch, params["nepochs"]):
        ...
        if params["save_model"]:
            if ep % params["savefreq"] == 0:
                torch.save(
                    model.state_dict(),
                    params["savedir"] + f"/model__epoch_{ep}_test_{params['name']}.pth",
                )
            ...
            for i, (x, p, ic) in enumerate(pbartest):
                ...
                if ep % params["logima_freq"] == 0:
                    ...
                    for i in range(len(x_pred.cpu())):
                        image = wandb.Image(
                            torch.tensor(np.float32(x_pred[i].cpu())), mode="F"
                        )
                        images.append(image)
                ...
                if ep == 0:
                    ...
                    for i in range(len(x.cpu())):
                        image = wandb.Image(
                            torch.tensor(np.float32(x[i].cpu())), mode="F"
                        )
                        images.append(image)

...
from discminer.tools.utils import InputError
...
def _log_prob_wemu(theta, **kwargs):
    return _GLOBAL_MODEL.ln_likelihood_wemu(theta, **kwargs)
...
class customDiscminerModel(Model):
        ...
        if self.first_makemodel:
            ...
            for side in ['upper', 'lower']:
                ...
                if self.Rmax_m is not None:
                    ...
                    self.R_grid[side] = R_grid #griddata((x_pro, y_pro), self.R_true, (self.mesh[0], self.mesh[1]), method='linear')
                ...
                if self.subpixels:
                    ...
                    for prop in props:
                        for i in range(self.subpixels_sq): #Subpixels are projected on the same plane where true grid is projected
                            ...
                            if self.Rmax_m is not None:
                                prop[i][side] = np.where(np.logical_and(self.R_grid[side]<self.Rmax_m, self.R_grid[side]>self.Rmin_m), prop[i][side], np.nan)
                    ...
                    for prop in props:
                        ...
                        if self.Rmax_m is not None: prop[side] = np.where(np.logical_and(self.R_grid[side]<self.Rmax_m, self.R_grid[side]>self.Rmin_m), prop[side], np.nan)
    ...
    def ln_likelihood(self, new_params, **kwargs):
        return super().ln_likelihood(new_params, **kwargs)
    ...
    def ln_likelihood_wemu(self, new_params, **kwargs):
            #t1 = time.time()
            torch.set_num_threads(1)
            for i in range(self.mc_nparams):
                if not (self.mc_boundaries_list[i][0] < new_params[i] < self.mc_boundaries_list[i][1]): return -np.inf

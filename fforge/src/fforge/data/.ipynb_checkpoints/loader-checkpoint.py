...
def get_labels_narray(base_path, labels, params):
...
def get_pretraining_data(base_path, n=10, image_size=128):
...
def generate_ict_mdeco(slopes, params, image_size=128):
...
def generate_ict_128x128_disc(slopes, params, nonorm=False, image_size=128):
...
def generate_ict_128x128_disc_tri(slopes, params, image_size=128):
    x = np.linspace(-3, 3, image_size)
    y = np.linspace(-3, 3, image_size)
    xx, yy = np.meshgrid(x, y)
    r = np.sqrt(xx**2+yy**2)
...
class TextImageDataset(Dataset):
    ...
    def __getitem__(self, ind):
        original_image = np.float32(self.data[ind])
        if self.mode!='mdeco':
            arr = self.params['norm'](original_image, self.params['scale'])
        else:
            arr = self.params['norm'](original_image, self.params['scale'])

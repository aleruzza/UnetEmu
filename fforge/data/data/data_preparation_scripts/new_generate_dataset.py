...
params = pd.read_csv('complete_dataset/run4.csv', index_col=0)
...
for i, row in tqdm(params.iterrows()):
    ...
    for filename in ['gasdens30.dat', 'gasvx30.dat', 'gasvy30.dat']:
        ...
        rho_d = np.fromfile(f'complete_dataset/out_{i:05}/{filename}').reshape(row['ny'].astype('int'), row['nx'].astype('int'))
...
np.save('gas_tri_ids_256.npy', np.array(ids))
np.save('gas_tri_256.npy', np.array(dataset))
